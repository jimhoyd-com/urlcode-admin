                    ...
                    try {
                        ...
                        if (request.method !== 'POST') {
                            if (path === '/' || path === '/dashboard') {
                                ...
                                const methodCounts=new Map<string,{signUps:number;signIns:number;failedSignIns:number}>();for(const day of stats?.daily??[])for(const entry of day.methods){const total=methodCounts.get(entry.method)??{signUps:0,signIns:0,failedSignIns:0};total.signUps+=entry.signUps;total.signIns+=entry.signIns;total.failedSignIns+=entry.failedSignIns;methodCounts.set(entry.method,total);}
                                const recent = hasPermission(principal, 'auth.audit.read') ? await service.listAudit({ limit: 20 }) : undefined;
                                return wantsJson(request) ? jsonResponse(200, { permissions: granted, csrf, ...(users ? { accounts: stats, accountsShown: users.users.length, moreAccounts: !!users.next } : {}), ...(recent ? { recentEvents: recent.events } : {}) }) : pageResponse('Administration', nav + (hasPermission(principal, 'auth.users.impersonate') && options.notifyImpersonation ? form(mount + '/impersonate', csrf, formField('accountId', 'Account ID') + formField('reason', 'Reason'), 'Start ten-minute support impersonation') : '') + '<p>Select a section. Only permitted operations are shown. Configuration remains in version-controlled project files.</p>' + (users ? `<p>${stats?.users} accounts; ${stats?.locked} locked; ${stats?.pendingDeletion} pending deletion; ${stats?.sessions} sessions; ${stats?.waitlist} waiting for approval.</p>` : '') + (stats?`<section aria-labelledby="daily-heading"><h2 id="daily-heading">Authentication activity: last 30 UTC days</h2><p>Recorded account creations, successful sign-ins and failed sign-ins. These figures describe authentication activity; deployment health is not measured here.</p><table><caption>Daily authentication counts</caption><thead><tr><th scope="col">UTC day</th><th scope="col">Sign-ups</th><th scope="col">Sign-ins</th><th scope="col">Failed sign-ins</th></tr></thead><tbody>${stats.daily.map(day=>`<tr><th scope="row">${escapeHtml(day.day)}</th><td>${day.signUps}</td><td>${day.signIns}</td><td>${day.failedSignIns}</td></tr>`).join('')}</tbody></table><table><caption>Authentication methods over the same 30 days</caption><thead><tr><th scope="col">Method</th><th scope="col">Sign-ups</th><th scope="col">Sign-ins</th><th scope="col">Failed sign-ins</th></tr></thead><tbody>${[...methodCounts].map(([method,total])=>`<tr><th scope="row">${escapeHtml(method)}</th><td>${total.signUps}</td><td>${total.signIns}</td><td>${total.failedSignIns}</td></tr>`).join('')}</tbody></table></section>`:'') + (recent ? `<h2>Recent events</h2><ul>${recent.events.map(event => `<li>${escapeHtml(event.action)} — ${escapeHtml(new Date(event.created).toISOString())}</li>`).join('')}</ul>` : ''));
                            }
                            if (path === '/cases') {
                            ...
                            if (path === '/users/detail') {
                                ...
                                return pageResponse('Account details', nav + `<dl><dt>Account ID</dt><dd>${escapeHtml(user.id)}</dd><dt>Email</dt><dd>${escapeHtml(user.email)}</dd><dt>Status</dt><dd>${escapeHtml(user.status)}</dd><dt>Email verified</dt><dd>${user.emailVerified ? 'Yes' : 'No'}</dd><dt>Authenticator enabled</dt><dd>${user.totpEnabled ? 'Yes' : 'No'}</dd><dt>Roles</dt><dd>${escapeHtml(user.roles.join(', '))}</dd><dt>Created</dt><dd>${escapeHtml(new Date(user.created).toISOString())}</dd><dt>Display name</dt><dd>${escapeHtml(user.profile?.displayName||'')}</dd><dt>Language</dt><dd>${escapeHtml(user.profile?.locale||'')}</dd><dt>Terms version</dt><dd>${escapeHtml(user.profile?.terms?.version||'')}</dd></dl>`+(activity?`<h2>Recent account activity</h2><ul>${activity.events.map(event=>`<li>${escapeHtml(event.action)} — ${escapeHtml(new Date(event.created).toISOString())}</li>`).join('')}</ul><a href="${escapeHtml(mount+'/audit?subject='+encodeURIComponent(user.id))}">View account audit</a>`:'') + (sessions ? `<p>${sessions.length} sessions.</p>` : '') + (hasPermission(principal, 'auth.users.export') ? form(mount + '/users/export', csrf, `<input type="hidden" name="accountId" value="${escapeHtml(user.id)}">` + formField('reason', 'Reason'), 'Export account data') : ''));
                            }
                            if (path === '/users') {
                                requirePermission(principal, 'auth.users.read');
                                ...
                                return pageResponse('Users', nav + (options.sendSetup && hasPermission(principal, 'auth.users.create') ? form(mount + '/users/create', csrf, formField('email', 'Email address', 'email') + formField('reason', 'Reason'), 'Create account and send setup link') : '') + `<form method="get" action="${escapeHtml(mount + '/users')}">${formField('query', 'Email contains', 'text', 'off', false)}${formField('role', 'Role name', 'text', 'off', false)}${formField('status', 'Status: active, locked or pending-delete', 'text', 'off', false)}<button>Filter users</button></form>` + (hasPermission(principal,'auth.users.export')?form(mount+'/users/export-page',csrf,['query','role','status','after'].map(key=>`<input type="hidden" name="${key}" value="${escapeHtml(request.query.get(key)||'')}">`).join('')+formField('reason','Reason')+'<p>Export this filtered page only, at most 50 accounts. Email addresses stay masked.</p>','Export this page as CSV'):'') + (hasPermission(principal,'auth.users.manage')||hasPermission(principal,'auth.sessions.manage')?`<form id="bulk-users" method="post" action="${escapeHtml(mount+'/users/bulk')}">${csrfField(csrf)}<label>Bulk action<select name="action">${hasPermission(principal,'auth.users.manage')?'<option value="lock">Lock</option><option value="unlock">Unlock</option>':''}${hasPermission(principal,'auth.sessions.manage')?'<option value="revoke-sessions">Revoke sessions</option>':''}</select></label>${formField('reason','Reason')}${formField('confirmation','Type LOCK, UNLOCK or REVOKE-SESSIONS followed by a space and the selected count')}<p>Select accounts in the table. All selected accounts must be permitted; otherwise none are changed.</p><button type="submit">Apply bulk action</button></form>`:'') + `<table><caption>Accounts (email addresses masked)</caption><thead><tr><th scope="col">Select</th><th scope="col">Account</th><th scope="col">Status</th><th scope="col">Roles</th><th scope="col">Actions</th></tr></thead><tbody>${users.map(user => `<tr><td>${user.id!==principal.id&&user.status!=='pending-delete'&&(hasPermission(principal,'auth.users.manage')||hasPermission(principal,'auth.sessions.manage'))?`<input type="checkbox" form="bulk-users" name="selected.${escapeHtml(user.id)}" value="yes" aria-label="Select ${escapeHtml(user.email)}">`:''}</td><td><a href="${escapeHtml(mount + '/users/detail?id=' + encodeURIComponent(user.id))}">${escapeHtml(user.email)}</a><br><code>${escapeHtml(user.id)}</code></td><td>${escapeHtml(user.status)}</td><td>${escapeHtml(user.roles.join(', '))}</td><td>${hasPermission(principal, 'auth.users.manage') && user.id !== principal.id && user.status !== 'pending-delete' ? form(mount + '/users/status', csrf, `<input type="hidden" name="accountId" value="${escapeHtml(user.id)}"><input type="hidden" name="status" value="${user.status === 'active' ? 'locked' : 'active'}">` + formField('reason', 'Reason'), user.status === 'active' ? 'Lock account' : 'Unlock account') : ''}</td></tr>`).join('')}</tbody></table>${result.next ? `<a href="${escapeHtml(nextPage(mount+'/users',request.query,result.next,['query','role','status']))}">Next page</a>` : ''}`);
                            }
                            if (path === '/roles') {
                            ...
                            if (path === '/audit') {
                                ...
                                return pageResponse('Audit', nav+`<form method="get" action="${escapeHtml(mount+'/audit')}">${['actor','subject','action'].map(key=>formField(key,key,'text','off',false)).join('')}${formField('from','From UTC (2026-01-01T00:00Z)','text','off',false)}${formField('to','To UTC (2026-01-02T00:00Z)','text','off',false)}<button>Filter audit</button></form>` + `<table><caption>Recent security events</caption><thead><tr><th scope="col">Time</th><th scope="col">Action</th><th scope="col">Actor</th><th scope="col">Subject</th></tr></thead><tbody>${result.events.map(event => `<tr><td>${escapeHtml(new Date(event.created).toISOString())}</td><td>${escapeHtml(event.action)}</td><td>${escapeHtml(event.actor)}</td><td>${escapeHtml(event.subject)}</td></tr>`).join('')}</tbody></table>${result.next ? `<a href="${escapeHtml(nextPage(mount+'/audit',request.query,result.next,['actor','subject','action','from','to']))}">Next page</a>` : ''}`);
                            }
                            throw new AuthHttpError(404, 'Not found');
                        }
                        const fields = readFields(request, [...selectedNames(request.body,request.headers.get('content-type')),'accountIds','confirmation','query','role','after','accountId', 'roles', 'status', 'reason', 'requestId', 'email', 'action', 'caseId', 'sessionId']);
                        ...
                        if(path==='/users/bulk'){
                            if(!['lock','unlock','revoke-sessions'].includes(fields.action||''))throw new AuthHttpError(400,'Invalid bulk action');
                            const action=fields.action as 'lock'|'unlock'|'revoke-sessions';requirePermission(principal,action==='revoke-sessions'?'auth.sessions.manage':'auth.users.manage');
                            const accountIds=selectedAccounts(fields);if(fields.confirmation!==action.toUpperCase()+' '+accountIds.length)throw new AuthHttpError(400,'Typed confirmation must match the action and selected count');
                            const result=await service.adminBulk({actorToken:token,accountIds,action,reason:fields.reason});return wantsJson(request)?jsonResponse(200,result):pageResponse('Bulk update completed',nav+`<p>${result.affected} accounts updated.</p>`);
                        }
                        if(path==='/users/export-page'){
                            requirePermission(principal,'auth.users.export');requirePermission(principal,'auth.users.read');const filterValues=new URLSearchParams();for(const key of ['query','role','status','after'])if(fields[key])filterValues.set(key,fields[key]);
                            const page=await service.listUsers(userFilters(filterValues));const users=[];for(const user of page.users){const exported=await service.adminExport({actorToken:token,accountId:user.id,reason:fields.reason});users.push(exported.user);}
                            const base=jsonResponse(200,{});return {...base,headers:[...base.headers.filter(([name])=>name!=='content-type'),['content-type','text/csv; charset=utf-8'],['content-disposition','attachment; filename="accounts-page.csv"'],...(page.next?[['x-next-cursor',page.next] as [string,string]]:[])],body:usersCsv(users)};
                        }
