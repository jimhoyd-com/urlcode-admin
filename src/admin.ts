                    ...
                    try {
                        ...
                        if (request.method !== 'POST') {
                            ...
                            if (path === '/users/detail') {
                                ...
                                return pageResponse('Account details', nav + `<dl><dt>${tr("field.accountId")}</dt><dd>${escapeHtml(user.id)}</dd><dt>${tr("copy.email")}</dt><dd>${escapeHtml(user.email)}</dd><dt>${tr("copy.status")}</dt><dd>${escapeHtml(user.status)}</dd><dt>${tr("copy.emailVerified")}</dt><dd>${tr(user.emailVerified ? 'common.yes' : 'common.no')}</dd><dt>${tr("copy.authenticatorEnabled")}</dt><dd>${tr(user.totpEnabled ? 'common.yes' : 'common.no')}</dd><dt>${tr("nav.roles")}</dt><dd>${escapeHtml(user.roles.join(', '))}</dd><dt>${tr("copy.created")}</dt><dd>${escapeHtml(new Date(user.created).toISOString())}</dd><dt>${tr("field.displayName")}</dt><dd>${escapeHtml(user.profile?.displayName || '')}</dd><dt>${tr("copy.language")}</dt><dd>${escapeHtml(user.profile?.locale || '')}</dd><dt>${tr("copy.termsVersion")}</dt><dd>${escapeHtml(user.profile?.terms?.version || '')}</dd></dl>` + (activity ? `<h2>${tr("copy.recentAccountActivity")}</h2><ul>${activity.events.map(event => `<li>${escapeHtml(event.action)} — ${escapeHtml(new Date(event.created).toISOString())}</li>`).join('')}</ul><a href="${escapeHtml(mount + '/audit?subject=' + encodeURIComponent(user.id))}">${tr("copy.viewAccountAudit")}</a>` : '') + (sessions ? `<p>${tr('message.sessionCount', { count: sessions.length })}</p>` : '') + (hasPermission(principal, 'auth.users.export') ? form(mount + '/users/export', csrf, `<input type="hidden" name="accountId" value="${escapeHtml(user.id)}">` + formField('reason', 'Reason'), 'Export account data') : ''));
                            }
                            if (path === '/users') {
                                requirePermission(principal, 'auth.users.read');
                        ...
                        if (path === '/users/export-page') {
                            ...
                            for (const key of ['query', 'role', 'status', 'after'])
                                if (fields[key])
