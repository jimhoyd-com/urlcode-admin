    ...
    return { name: 'admin', version: '1', projectSha256: options.projectSha256, targets: ['node'], schema, credentialHeaders: ['cookie', 'authorization', 'x-csrf-token'],
        activate(_config, context) {
            ...
            function navigation(principal: AuthPrincipal, text: (source: string) => string, tr: (key: string) => string, current: string): string {
                ...
                const contents=`<nav aria-label="${tr('page.admin')}">${link('/',tr('nav.overview'))}${[['users','Users','auth.users.read'],['sessions','Sessions','auth.sessions.manage'],['registrations','Registration','auth.users.manage'],['roles','Roles','auth.roles.read'],['audit','Audit','auth.audit.read'],['cases','Cases','auth.cases.read'],['health','Service health','auth.health.read']].filter(([_path,_label,permission])=>hasPermission(principal,permission!)).map(([path,label])=>link('/'+path,escapeHtml(text(label!)))).join('')}${accounts.enabled()&&hasPermission(principal,'auth.users.manage')?link('/account-operations',tr('adminOps.title')):''}${recovery.enabled()&&hasPermission(principal,'auth.cases.read')?link('/recovery-cases',tr('manualRecovery.title')):''}</nav><div class="ui-sidebar-footer"><a class="ui-nav-link" href="${escapeHtml(authMount+'/account')}">${tr('nav.account')}</a><a class="ui-nav-link" href="${escapeHtml(authMount+'/step-up')}">${tr('action.confirm')}</a></div>`;
                return `<aside class="ui-sidebar"><a class="ui-brand" href="${escapeHtml(mount)}"><span aria-hidden="true">U</span><strong>URLCode</strong></a><div class="ui-desktop-navigation"><p class="ui-muted">${tr('page.admin')}</p>${contents}</div><details class="ui-mobile-navigation"><summary>${tr('page.admin')}</summary>${contents}</details></aside>`;
            }
