...
export function createAdminPresentation(options:Omit<PresentationOptions,'defaults'>={}):Presentation {
 ...
 return Object.freeze({locales:base.locales,resolve(preferences?:LocalePreferences){const context=base.resolve(preferences),copy=own.resolve(preferences);return Object.freeze({...context,textSource(source:string){const key=sources.get(source);return key?copy.text(key):context.textSource(source);},text(key:string,values?:Readonly<Record<string,string|number>>){return isAdmin(key)?copy.text(key,values):context.text(key,values);}});}});
}
