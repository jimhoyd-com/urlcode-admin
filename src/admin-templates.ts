...
const screens:Record<string,{body:string;sample:ViewModel}>={
 ...
 sessions:{
  body:`<section class="ui-card"><h2>{{filtersHeading}}</h2><p class="ui-muted">{{intro}}</p>{{filters}}</section><section class="ui-card"><p class="ui-muted">{{count}}</p>{{#if table}}{{table}}{{else}}<p class="ui-empty">{{empty}}</p>{{/if}}${next}</section>{{#if revokeAll}}<details class="ui-card ui-danger-zone"><summary>{{revokeAll.summary}}</summary><p class="ui-muted">{{revokeAll.help}}</p>{{revokeAll.form}}</details>{{/if}}`,
 ...
 roles:{
  body:`<section class="ui-card"><p>{{intro}}</p><p class="ui-muted">{{note}}</p>{{table}}</section>{{#if assign}}<section class="ui-card"><h2>{{assign.heading}}</h2>{{assign.form}}</section>{{/if}}`,
 ...
 audit:{
  body:`<section class="ui-card"><h2>{{filtersHeading}}</h2><p class="ui-muted">{{intro}}</p>{{filters}}</section><section class="ui-card">{{#if table}}{{table}}{{else}}<p class="ui-empty">{{empty}}</p>{{/if}}${next}</section>{{#if export}}<details class="ui-card"><summary>{{export.summary}}</summary>{{export.form}}</details>{{/if}}`,
 ...
 health:{
  body:`{{#if unavailable}}${empty('unavailable')}{{else}}<section class="ui-card"><p class="ui-muted">{{intro}}</p><dl class="ui-definition-grid">{{#each facts}}<dt>{{term}}</dt><dd>{{value}}</dd>{{/each}}</dl></section><section class="ui-card">{{components}}</section><section class="ui-card"><h2>{{alertsHeading}}</h2>{{#if alerts}}<ul class="ui-list">{{#each alerts}}<li>{{this}}</li>{{/each}}</ul>{{else}}<p class="ui-empty">{{noAlerts}}</p>{{/if}}</section>{{/if}}`,
