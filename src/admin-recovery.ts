...
export function createAdminRecovery(options:AdminRecoveryOptions,http:AuthHttp,mount:string){
 ...
 return {enabled,async handle(request:ExtensionRequest,principal:AuthPrincipal,actorToken:string,presentation:PresentationContext,nav:string):Promise<Awaited<ReturnType<ExtensionInstance['handle']>>|undefined>{
  ...
  if(request.method==='GET'||request.method==='HEAD'){
   ...
   if(wantsJson(request))return jsonResponse(200,{...result,csrf});
   const canManage=hasPermission(principal,'auth.cases.manage');
   const cases=result.cases.map(item=>`<li><h2>${escapeHtml(item.id)}</h2><dl><dt>${html('account')}</dt><dd>${escapeHtml(item.accountId)}</dd><dt>${html('address')}</dt><dd>${escapeHtml(item.recovery.email)}</dd><dt>${html('status')}</dt><dd>${html('state.'+item.recovery.state)}</dd><dt>${html('evidence')}</dt><dd>${escapeHtml(item.recovery.evidence.summary)}</dd><dt>${html('reference')}</dt><dd>${escapeHtml(item.recovery.evidence.reference||tr('none'))}</dd></dl><p>${escapeHtml(item.reason)}</p><ul>${(item.notes??[]).map(note=>`<li>${escapeHtml(note.actorId)}: ${escapeHtml(note.note)}</li>`).join('')}</ul>${canManage?form('/recovery-cases/note',csrf,hidden(item.id)+formField('reason',tr('note')),tr('addNote')):''}${canManage&&item.status==='pending'?form('/recovery-cases/close',csrf,hidden(item.id)+formField('reason',tr('closure')),tr('close')):''}${canManage&&item.status==='pending'&&item.makerId!==principal.id?form('/recovery-cases/approve',csrf,hidden(item.id)+formField('reason',tr('approval'))+formField('confirmation',tr('confirmation')),tr('approve')):''}</li>`).join('');
  ...
  if(path==='/recovery-cases/create')await options.service.createRecoveryCase({actorToken,accountId:fields.accountId||'',email:fields.email||'',evidence:{summary:fields.summary||'',...(fields.reference?{reference:fields.reference}:{})},reason:fields.reason});
   ...
   try{
    ...
    await Promise.race([options.sendRecovery!({email:issued.email,oldEmail:issued.oldEmail,token:issued.token,caseId:issued.case.id,signal:controller.signal}),new Promise<void>((_,reject)=>{timer=setTimeout(()=>{controller.abort();reject(new Error('Recovery delivery timed out'));},5000);})]);
   ...
   finally{if(timer)clearTimeout(timer);delivering--;}
