...
export function createAdminAccount(options:AdminAccountOptions,http:AuthHttp,mount:string){
 ...
 return {enabled:()=>!!options.sendAccountAdministration,async handle(request:ExtensionRequest,principal:AuthPrincipal,actorToken:string,presentation:PresentationContext,nav:string):Promise<Awaited<ReturnType<ExtensionInstance['handle']>>|undefined>{
  ...
  if(request.method==='GET'||request.method==='HEAD'){
   ...
   return pageResponse(tr('title'),nav+`<section class="ui-card"><p class="ui-muted">${escapeHtml(presentation.textSource('Account ID'))}: <code>${escapeHtml(accountId)}</code></p><p>${html('info')}</p><dl class="ui-definition-grid"><dt>${html('password')}</dt><dd>${html(methods.password?'yes':'no')}</dd><dt>${html('totp')}</dt><dd>${html(methods.totp?'yes':'no')}</dd></dl></section><ul class="ui-list">${methods.passkeys.map(method=>`<li class="ui-card"><strong>${html('passkey')}</strong> <code>${escapeHtml(method.id)}</code> ${dates(method)} ${method.secondFactor?html('secondFactor'):manage?form('remove-passkey',accountId,hidden('credentialId',method.id)):''}</li>`).join('')}${methods.external.map(method=>`<li class="ui-card"><strong>${escapeHtml(method.provider)}</strong> ${dates(method)}${manage?form('remove-external',accountId,hidden('externalId',method.id)):''}</li>`).join('')}</ul>${!methods.passkeys.length&&!methods.external.length?`<p class="ui-empty">${escapeHtml(presentation.textSource('No passkeys or external sign-in methods recorded.'))}</p>`:''}<p><a class="ui-button-secondary" href="${escapeHtml(mount+'/cases')}">${html('factorCase')}</a></p>`+(manage?['verify-email','resend-verification','force-password-reset','schedule-deletion','cancel-deletion'].map(action=>form(action as AdminAccountAction,accountId)).join('')+form('request-email-change',accountId,formField('email',tr('email'),'email'))+form('assign-roles',accountId,formField('roles',tr('roles'))):''),200,[],undefined,presentation);
  }
