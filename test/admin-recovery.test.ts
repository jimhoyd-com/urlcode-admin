...
test('manual recovery admin escapes evidence and requires explicit confirmation before private delivery',async()=>{
 ...
 const page=await helper.handle(request('/recovery-cases'),principal,actorToken,context,'');assert.equal(page?.status,200);assert.match(Buffer.from(page?.body??'').toString(),/&lt;img/);assert.doesNotMatch(Buffer.from(page?.body??'').toString(),/<img src=x/);assert.doesNotMatch(Buffer.from(page?.body??'').toString(),/href="https:\/\/internal/);
