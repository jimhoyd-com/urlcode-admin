...
test('real admin handler stages account mutation until notice succeeds and records per-user audit',async t=>{
 ...
 const instance=await adminExtension({service,csrfKey,projectSha256,sendAccountAdministration:async message=>{assert.equal((await service.getUser(target.user.id))?.emailVerified,false);assert.equal(message.email,target.user.email);if(fail)throw new Error('Notice rejected');delivered++;}}).activate({}, {origin,target:'node',projectSha256,mounts:['/admin']});
