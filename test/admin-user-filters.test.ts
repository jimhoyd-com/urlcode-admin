...
test('admin pages and CSV export preserve expanded filters, masked results and read/export permissions', async (t) => {
    ...
    const instance = await adminExtension({ service: client, csrfKey, projectSha256, ...(ui ? { ui } : {}) }).activate({}, { origin, target: 'node', projectSha256, mounts: ['/admin'] });
...
test('real user-query pagination and page export preserve combined filters across more than fifty accounts', async (t) => {
    ...
    const origin = 'https://example.test', csrfKey = randomBytes(32), projectSha256 = 'b'.repeat(64), http = new AuthHttp({ origin, csrfKey }), ui = await activatedUi(t, renderOf(t), root, projectSha256), instance = await adminExtension({ service, csrfKey, projectSha256, ...(ui ? { ui } : {}) }).activate({}, { origin, target: 'node', projectSha256, mounts: ['/admin'] });
