...
Status: `0.1.0-alpha.3`, published through the tag-driven release workflow after core `0.4.0-alpha.2`, ui `0.1.0-alpha.5` and auth `0.1.0-alpha.3`. It supersedes `0.1.0-alpha.2`, the first working npm alpha (`0.1.0-alpha.1` was blocked by npm's registry after an earlier out-of-band publish attempt and could never be used), and carries the work merged since — the trusted project-level lifecycle hooks and the `ExtensionActivation.root` fixture fixes for core 0.4.0-alpha.2 (core issue #78). The implemented admin workflows and reviewed shared-presentation work are merged to main. Production release validation remains separate. Cross-repository acceptance: https://github.com/jimhoyd-com/urlcode/issues/58. Core extension integration PR #59 is merged. Auth owns identities, sessions and transactional authority checks; this package owns the trusted administration interface.
